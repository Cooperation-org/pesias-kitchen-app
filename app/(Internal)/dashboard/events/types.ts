...
export interface ServerEvent {
  ...
  participants: Participant[]; // populated users
  createdBy: {
    _id: string;
    walletAddress: string;
    name: string;
  ...
  qrCodes?: {
    volunteer: string | null; // QRCode ObjectId as string
...
export interface ProcessedEvent {
  ...
  location: string;
  /**
   * ISO string for full date-time (same as ServerEvent.date)
   */
  date: string;
  /**
   * Preformatted "HH:mm" (derived in mapper for convenience)
   */
  time: string;
  activityType: ActivityType;
  capacity: number;
